...
// TODO: Replace `Decimal` with `rust_decimal::Decimal` totally. 
impl From<Decimal> for rust_decimal::Decimal {
    fn from(decimal: Decimal) -> Self {
        rust_decimal::Decimal::from_i128_with_scale(decimal.underlying, decimal.scale as u32)
    }
}
...
impl Decimal {
    ...
    pub fn new(underlying: i128, scale: u8) -> Decimal {
        let precision = 38;
        if scale > precision {
            panic!("scale can't be greater than 38");
        }
